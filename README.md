...
[1] Retrieve  — 向量检索 top-k 候选片段
  │
  ▼
[2] Grade     — LLM 评估每个片段与问题的相关性和置信度（输出 JSON）
  │
  ▼
[3] Decide    — 条件分叉
...
Grade 节点使用 `response_format={"type": "json_object"}` 强制模型输出结构化 JSON，消除 Markdown 包裹导致的解析失败。每个片段会输出 `relevant` 和 `confidence`，只有同时满足 `relevant=true` 且置信度不低于 `DOCAGENT_MIN_RELEVANCE_CONFIDENCE`（默认 `0.55`）的片段才会进入生成。这样可以避免“模型觉得有点相关，但证据很弱”时直接硬答。
